...
function visiblePeople(){const s=$('search')?low($('search').value):'';const sess=$('sessionFilter')?.value||'all';const type=$('typeFilter')?.value||'all';const stat=$('statusFilter')?.value||'all';return people.filter(p=>{if(type!=='all'&&p.type!==type)return false;if(sess!=='all'&&p.type==='Public'&&low(p.session)!==low(sess))return false;if(stat==='checked'&&!p.checked)return false;if(stat==='notchecked'&&p.checked)return false;if(stat==='waiver'&&p.waiver)return false;if(s&&!low([p.name,p.parent,p.email,p.phone,p.team,p.session,p.memberId,p.qr].join(' ')).includes(s))return false;return true;});}
